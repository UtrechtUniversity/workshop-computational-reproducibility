...
scientist-coders can benefit from learning about modularity, abstraction, and data structures, if they want to leverage their past discoveries into further data discoveries without having to reinvent the wheel every time. (https://www.software.ac.uk/blog/2016-09-26-scientific-coding-and-software-engineering-whats-difference)
...
```
  - Example (in R)
```{R}
read_vector_value <- function(index=1){
  my_vector <- c(10,5,4,12,25)
  
  if(index>length(my_vector)){
    stop("Index cannot be higher than length of the vector")
  }
return(my_vector[index])
}
```
