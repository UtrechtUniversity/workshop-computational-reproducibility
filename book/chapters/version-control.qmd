...
Let us open the README file again. You can do by opening a commandline editor like nano or vim (Mac, linux) or you can go to your file browser and open the README file with e.g. Notepad or VS Code.
...
```
git add <path of the file as shown in status>
```
...
`git clone` is a one time action to get a copy of an online repository on GitHub that is not present yet on your PC. `git pull` is done to update the repository with any (potential) changes to the online copy of this repository on GitHub.
## The .gitignore file
...
- We explored the GitHub web interface to setup and manage a project.
- We used a "template repository" to initiate a repository with a predefined file folder structure and some default files to make life a bit easier.
